...
def debugimg(img, debug):
    if debug:
        imshow("debug", cvtColor(img, COLOR_RGB2BGR))
        waitKey(0)
        destroyAllWindows()


def debugtext(text, debug):
    if debug:
        STYLES.DEBUG.print(text)


def subregion(img, base, target, debug):
    subimg = img[target[1] - base[1] : target[3] - base[1], target[0] - base[0] : target[2] - base[0]]
    debugimg(subimg, debug)
...
class Trans:
    patd = re.compile(r"[0-9]+[.,]{0,1}[0-9]*")

    def get_double(text):
        res = Trans.patd.findall(text)
        if len(res) != 1:
            return None
        return double(res[0])
...
def put_stat(key, value, statmap, debug=False, value_override=None):
    ...
    standardized_value = value_override if value_override is not None else Trans.get_double(value)
    ...
    debugtext("%s : %.2f" % (standardized_key, standardized_value), debug)
    statmap[standardized_key] = standardized_value


def summarize(stats, level, debug):
    statmap = dict(stats)
    ...
    STYLES.CRIT_RATE.print("CRIT SCORE: %.2f (%.2f hits)" % (crit_score, crit_score / (HITS.CRIT_DMG * double("0.85"))))

    atk_hits = statmap.get(NAMES.CRIT_RATE, double("0.0")) / (HITS.CRIT_RATE * double("0.85")) * ATK_WEIGHTS.CRIT_RATE
    ...
    def_hits = statmap.get(NAMES.CRIT_RATE, double("0.0")) / (HITS.CRIT_RATE * double("0.85")) * DEF_WEIGHTS.CRIT_RATE
...
def understanding(img, debug):
    debugimg(img, debug)
    statmap = {}
    # Main Stats
    main_key = subregion(img, regions.shot, regions.main_key, debug)
    main_val = subregion(img, regions.shot, regions.main_val, debug).replace(",", "")
    put_stat(main_key, main_val, statmap, debug, value_override=double("0.0"))
    # TODO: Detect the number of stars
    # subregion(img, regions.shot, regions.stars, debug)
    # Level
    level_str = subregion(img, regions.shot, regions.level, debug)
    level = Trans.get_int(level_str)
    # Sub Stats
    for sub in regions.subs:
        sub_kv = subregion(img, regions.shot, sub, debug)
        if tailing(sub_kv):
            break
        ...
        put_stat(sub_key, sub_value, statmap, debug)
    # Summarize Current
    curr_id = hash(frozenset(statmap.items()))
    if curr_id != SESSION.prev_id:
        summarize(statmap, level, debug)
        SESSION.prev_id = curr_id


def parser(debug=False):
    while True:
        img = screenshot()
        understanding(img, debug)
        if debug:
            break
        sleep(0.100)
