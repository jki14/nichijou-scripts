    ...
    def __init__(
        ...
        if full is None:
            ...
            self.full: ScreenRegion = (
                ...
                min(main_key[1], main_val[1], stars[1], level[1], *(s[1] for s in substats)) + min(dy, 0),
                max(main_key[2], main_val[2], stars[2], level[2], *(s[2] for s in substats)),
                max(main_key[3], main_val[3], stars[3], level[3], *(s[3] for s in substats)) + max(dy, 0),
            )
            self.dy: int = dy
...
G3440MO = G3440.translated(-934, 70)

G3440CA = RegionsPrfl(full=(2840, 180, 3294, 624))
...
H3440 = RegionsPrfl(
    main_key=(2920, 390, 3260, 440),
    main_val=(3260, 390, 3388, 440),
    stars=(2868, 330, 2938, 370),
    level=(2868, 330, 2938, 370),
    substats=[(2920, 450, 3388, 500, 3260), (2920, 504, 3388, 550, 3260), (2920, 554, 3388, 600, 3260), (2920, 606, 3388, 650, 3260)],
)

Z3440 = RegionsPrfl(
    main_key=(1190, 378, 1550, 418),
    main_val=(1550, 378, 1900, 418),
    stars=(1340, 250, 1500, 290),
    level=(1340, 250, 1500, 290),
    substats=[(1190, 500, 1900, 540, 1550), (1190, 570, 1900, 610, 1550), (1190, 640, 1900, 680, 1550), (1190, 710, 1900, 750, 1550)],
)
